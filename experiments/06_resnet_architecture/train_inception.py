"""
Independent training script for Inception-CIFAR on CIFAR-10.
...
from inception import build_inception_cifar, save_inception_weights
...
def train_inception(
    ...
    """
    Trains Inception-CIFAR on CIFAR-10 and saves its weights.
    """
    if weights_path is None:
        weights_path = os.path.join(current_dir, "weights", "inception_cifar10.weights.h5")
    if history_path is None:
        history_path = os.path.join(current_dir, "weights", "inception_history.json")

    print("=" * 60)
    print(" 🚀 Training Inception-CIFAR on CIFAR-10")
    print("=" * 60)
    ...
    # 2. Build Model
    model = build_inception_cifar(input_shape=(32, 32, 3), num_classes=10)
    print(f" Inception Parameters: {model.count_params():,}")
    ...
    print(f"\n🌐 Inception Test Accuracy: {test_acc * 100:.2f}% | Test Loss: {test_loss:.4f} (Elapsed: {duration:.1f}s)")

    saved_path = save_inception_weights(model, weights_path)
    print(f"💾 Stored Inception weights to: {saved_path}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train Inception-CIFAR on CIFAR-10")
