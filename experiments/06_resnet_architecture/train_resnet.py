"""
Independent training script for ResNet-20 on CIFAR-10.
...
from resnet import build_resnet20, save_resnet_weights
...
def train_resnet(
    ...
    """
    Trains ResNet-20 on CIFAR-10 and saves its weights.
    """
    if weights_path is None:
        weights_path = os.path.join(current_dir, "weights", "resnet20_cifar10.weights.h5")
    if history_path is None:
        history_path = os.path.join(current_dir, "weights", "resnet_history.json")
    ...
    print(f" ResNet-20 Parameters: {model.count_params():,}")
    ...
    print(f"\n🏆 ResNet-20 Test Accuracy: {test_acc * 100:.2f}% | Test Loss: {test_loss:.4f} (Elapsed: {duration:.1f}s)")

    saved_path = save_resnet_weights(model, weights_path)
    print(f"💾 Stored ResNet-20 weights to: {saved_path}")
