"""
Independent training script for EfficientNet on CIFAR-10.
...
from efficientnet import build_efficientnet_cifar, save_efficientnet_weights
...
def train_efficientnet(
    ...
    """
    Trains EfficientNet on CIFAR-10 and saves its weights.
    """
    if weights_path is None:
        weights_path = os.path.join(current_dir, "weights", "efficientnet_cifar10.weights.h5")
    if history_path is None:
        history_path = os.path.join(current_dir, "weights", "efficientnet_history.json")

    print("=" * 60)
    print(" 🚀 Training EfficientNet-CIFAR on CIFAR-10")
    print("=" * 60)
    ...
    # 2. Build Model
    model = build_efficientnet_cifar(input_shape=(32, 32, 3), num_classes=10)
    print(f" EfficientNet Parameters: {model.count_params():,}")
    ...
    print(f"\n⚡ EfficientNet Test Accuracy: {test_acc * 100:.2f}% | Test Loss: {test_loss:.4f} (Elapsed: {duration:.1f}s)")

    saved_path = save_efficientnet_weights(model, weights_path)
    print(f"💾 Stored EfficientNet weights to: {saved_path}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train EfficientNet on CIFAR-10")
